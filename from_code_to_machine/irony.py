...
def compile_stmt(stmt, output):
    if stmt["type"] == "assign":
        ...
        output.append(("MOV", start_tmp, stmt["var"]))
        
        # Store end value
        output.extend(end_code)
        end_var = temp()
        output.append(("MOV", end_tmp, end_var))
        
        # Determine direction: check if start > end
        check_tmp = temp()
        output.append(("SUB", start_tmp, end_var, check_tmp))
        output.append(("JGE", check_tmp, 1, f"{loop_label}_down"))
        
        # Ascending loop (start <= end)
        output.append(("LABEL", f"{loop_label}_start"))
        output.append(("JGT", stmt["var"], end_var, f"{end_label}_end"))
        
        for s in stmt["body"]:
            compile_stmt(s, output)
        
        inc_tmp = temp()
        output.append(("ADD", stmt["var"], 1, inc_tmp))
        output.append(("MOV", inc_tmp, stmt["var"]))
        output.append(("JMP", f"{loop_label}_start"))
        
        # Descending loop (start > end)
        output.append(("LABEL", f"{loop_label}_down"))
        output.append(("JGT", end_var, stmt["var"], f"{end_label}_end"))
        
        for s in stmt["body"]:
            compile_stmt(s, output)
        
        dec_tmp = temp()
        output.append(("SUB", stmt["var"], 1, dec_tmp))
        output.append(("MOV", dec_tmp, stmt["var"]))
        output.append(("JMP", f"{loop_label}_down"))
        
        output.append(("LABEL", f"{end_label}_end"))
